 ...
 * 中断：
 * <p>
 * 1. 当线程中有方法执行了wait，sleep，join，await方法陷入阻塞时，如果当前线程调用thread.interrupt()方法，会将线程设置为
 * 中断状态，这些阻塞方法就会抛出中断异常，并且同时重置中断标识位。
 * <p>
 * 2. Thread.interrupted() 即(currentThread().isInterrupted(true)) 判断当前线程是否是中断了，如果中断了，则会返回true，
 * 并且同时清理线程的中断标识。 该方法使用于线程本身没有阻塞方法并且也需要校验自己是否被外部调用中断了。
 *
 * @author: mahao
 * @date: 2019/8/10
 */

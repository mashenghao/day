package collection.map.src.jdk8;
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
     ...
     */
    //散列hashcode的值，让高16位和低16位异或，让hashcode每位都能使用
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        if (key == null || value == null) throw new NullPointerException(); //不支持key和value为null的情况
        int hash = spread(key.hashCode());//散列hashcode
